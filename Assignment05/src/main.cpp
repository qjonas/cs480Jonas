...
bool initialize()
{
   ...
   // assimp model loading
    ...
    const aiScene* modelScene = importer.ReadFile(objFname, aiProcess_Triangulate);
     
     // load details from scene
        Vertex* geometry = assimpLoader( modelScene );
...
// cout << "face created "  << endl;
		for( k = 0; k < 3; k++ ) // get the three indices from the face
		{
//		cout << "in k loop" << endl;
...
// cout << "pos created "  << endl;
		 ...
		 ret_model[j].color[0] = .8;
		 ret_model[j].color[1] = 1;		 
		 ret_model[j].color[2] = 0.08;
